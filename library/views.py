...
from student.models import Utilisateur
...
def issuerequest(request,bookID):
    student=Utilisateur.objects.filter(student_id=request.user)
    if student:
        book=Book.objects.get(id=bookID)
...
def myissues(request):
    if Utilisateur.objects.filter(student_id=request.user):
        student=Utilisateur.objects.filter(student_id=request.user)[0]
...
def requestedissues(request):
    if request.GET.get('studentID') is not None and request.GET.get('studentID') != '':
        try:
            ...
            student=Utilisateur.objects.filter(student_id=user)
            if student:
                student=student[0]
                issues=Issue.objects.filter(student=student,issued=False)
